...
- 実機の公式LINE（`/line`）では、会話の要約と条件整理をサーバー側の `POST /api/line/analyze` で行う。AIが埋めた値は必ず`inferred`で返し、**根拠になった顧客の発言を`quote`として同梱する**。引用が取れない値は採用せず`unknown`のままにする。値だけが埋まって根拠が無い状態を作らない。
